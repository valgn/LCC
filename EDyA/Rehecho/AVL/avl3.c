...
typedef void (*FuncionVisitanteExtra)(void *dato);
...
static AVL_Nodo* rebalance(AVL_Nodo* raiz){
...
static AVL_Nodo* avl_nodo_eliminar(AVL_Nodo* raiz, void* dato, FuncionComparadora cmp, FuncionDestructora destroy){
    if(raiz == NULL) return NULL;
    int c = cmp(dato, raiz->dato);
    if(c < 0){
        raiz->izq = avl_nodo_eliminar(raiz->izq, dato, cmp, destroy);
    } 
    else if(c > 0){
        raiz->der = avl_nodo_eliminar(raiz->der, dato, cmp, destroy);
    }
    else{
        if(!raiz->izq || !raiz->der){
            AVL_Nodo* sucesor = raiz->der == NULL ? raiz->izq : raiz->izq;
            destroy(raiz->dato);
            free(raiz);
            return sucesor;
        }
            ...
            raiz->der = avl_nodo_eliminar(raiz->der, sucesor->dato, cmp, destroy);
            raiz->altura = 1 + avl_nodo_max_altura_hijos(raiz);
...
int main(){
    ...
    for(int i = 1; i <= 10; i++){
        ...
        printf("Insertado %d | Altura Raiz: %d | Balance Raiz: %d\n", i, avl_nodo_altura(arbol->raiz), 
        avl_nodo_factor_balance(arbol->raiz));
    }
    printf("La raiz del arbol es: "); imprimir_entero(arbol->raiz->dato);
    avl_recorrer(arbol, AVL_RECORRIDO_IN, imprimir_entero);
    ...
    for(int i = 0; i < 3; i++){
        ...
        avl_recorrer(arbol, 0,imprimir_entero);
        printf("--------------------------------\n");
    }
