...
int comparar_contacto(Contacto* c1, Contacto* c2){
    if(c1->edad < c2->edad){
        return 1;
    }
    else return 0;
...
int igual_contacto(Contacto* c1, Contacto* c2){
    if(c1->edad == c2->edad && strcmp(c1->tel,c2->tel) && strcmp(c1->nombre,c2->nombre)){
        return 1;
    }
    else return 0;
}