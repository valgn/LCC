...
void avl_destruir(AVL arbol, FuncionDestructora destroy) {
  avl_nodo_destruir(arbol->raiz, destroy);
  free(arbol);
}
...
int avl_buscar(AVL arbol, void* dato, FuncionComparadora comp) {
  return avl_nodo_buscar(arbol->raiz, comp, dato);
}

static void* avl_nodo_buscar_retornar(AVL_Nodo* raiz, FuncionComparadora comp, void* dato){
  if(raiz == NULL) return NULL;
  if(comp(dato, raiz->dato) == 0) return raiz->dato;
  else if(comp(dato, raiz->dato) < 0) return avl_nodo_buscar_retornar(raiz->izq, comp, dato);
  else{
...
static AVL_Nodo* eliminar_nodo(AVL_Nodo* raiz,void* dato ,FuncionComparadora comp, FuncionCopiadora copy, FuncionDestructora destroy){
  ...
  if(c < 0){
    raiz->izq = eliminar_nodo(raiz->izq, dato, comp, copy, destroy);
  }
  else if(c > 0){
    raiz->der = eliminar_nodo(raiz->der, dato, comp, copy, destroy);
  }
  else
  {// Encontramos el dato, veamos sus hijos.
    ...
    {// Debemos encontrar al menor de la raiz derecha.
      AVL_Nodo* sucesor = raiz->der;
      while(sucesor != NULL) sucesor = sucesor->izq;
      void* copia_dato = raiz->dato;
...
void avl_eliminar(AVL arbol, void* dato, FuncionComparadora comp, FuncionCopiadora copy, FuncionDestructora destroy){
  arbol->raiz = eliminar_nodo(arbol->raiz, dato, comp, copy, destroy);
}
